...
- 🎙 **录制**：加载 `.md` / `.pdf`（或选择含 `images/` 的 md 项目文件夹），配合麦克风录制讲解；
  自动记录页面滚动位置、文字选中、鼠标点击，停止后导出 `lecture.zip`
- ▶️ **播放**：加载 zip 后按音频时间轴自动滚动页面、还原光标与点击效果，支持拖动进度、0.75×–2× 变速，空格暂停/继续
- 🔗 **分享**：配合 `serve.py` 把 zip 上传到局域网服务，一键生成播放链接发给同事
- 📦 **自包含**：zip 内包含源文件 + 音频 + 时间轴数据（`lecture.json`），20 分钟讲解约 15 MB
...
1. **录制**：切到「录制」→ 选择文件 / 文件夹 → 开始录制（允许麦克风）→ 边讲边滚动、选词、点击 → 停止后导出或分享
2. **播放**：切到「播放」→ 选择 `lecture.zip` → 播放
3. **分享**：点「分享」生成局域网链接，同事打开即自动加载并播放
...
├── 源文件（md / pdf / images/ …）
├── audio.webm
└── lecture.json    # 时间轴：states（滚动/选词）+ cursor + clicks
```
